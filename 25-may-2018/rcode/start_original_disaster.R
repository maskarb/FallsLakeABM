...
#    params <- c("observedInflow", "outflow", "waterSupply", "storage", "elevation" )
...
#                                        population, shiftFactor, folder, filename)
    ...
    for (param in params) {
        if (param != "storage" | param != "elevation") {
            ...
            parameter <- stress_ts[, param] / 131395 * 100
        } else {
        ...
        file_path <- sprintf("~/MASON/25-may-2018/%s_%s.csv", folder, param)
        if(file.exists(file_path)){
        write_csv(para.df, file_path, append=TRUE, col_names=FALSE)
        } else {
        write_csv(para.df, file_path, append=TRUE, col_names=TRUE)
        }
    }
  }
}