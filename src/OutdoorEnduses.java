...
public class OutdoorEnduses {
	...
	public OutdoorEnduses(int month, double area, double wateringFreq, boolean isWarmCrop, double conservationFactor,
			Climate climate) {
		double[] warmTurfCoefficient = { 0.55, 0.54, 0.76, 0.72, 0.79, 0.68, 0.71, 0.71, 0.62, 0.54, 0.58, 0.55 };

		double[] coldTurfCoefficient = { 0.61, 0.64, 0.75, 1.04, 0.95, 0.88, 0.94, 0.86, 0.74, 0.75, 0.69, 0.6 };
	...
	public void calculateOutdoorDemand(int month, double conservationFactor) {
		...
		if (climate.getEvapotrans(month) <= climate.getPrecip(month)) {
			// gardenWatering = 0;
			gardenWatering = -0.1 * conservationFactor * (climate.getEvapotrans(month) - climate.getPrecip(month))
					* area * cropCoefficient[monthNum];
			...
			gardenWatering = conservationFactor * wateringFreq * (0.624 / 0.71)
					* (climate.getEvapotrans(month) - climate.getPrecip(month)) * area * cropCoefficient[monthNum];
		}
