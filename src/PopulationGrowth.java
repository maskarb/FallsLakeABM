...
public class PopulationGrowth implements Steppable {
  ...
  public PopulationGrowth(
      PrintWriter outputStreamPopGrowth,
      PrintWriter outputStreamHousehold,
      String scenario,
  ...
  public void step(SimState state) {
    ...
    // double[] area = { 692123097.6, 1121157734, 1422941414, 1688544507, 2870094335.0,
    // 3416725186.0, 3758397705.0, 3946317590.0, 4143633469.0 };
    ...
    double[] area = {
      1688544507, 2870094335.0, 3416725186.0, 3758397705.0, 3946317590.0, 4143633469.0
    };
    ...
    if (!garbage) {
      ...
      increasedPop =
          (population[g + 1] - population[g]) * ((time - g * 120) / 120d) + population[g];
      ...
      if (householdTracker != totalNumOfHouseholds && !garbage) {
        System.out.println(
            householdArea
                ...
                + increasedPop
                + " "
                + (totalNumOfHouseholds - oldTotalNumOfHouseHold)
                + " "
                + totalNumOfHouseholds
                + " "
                + (population[g + 1] - population[g])
                + " "
                + population[g]
                + " "
                + population[g + 1]
    ...
    for (int i = 0; i < totalNumOfHouseholds - oldTotalNumOfHouseHold; i++) {
      ...
      IndoorEnduses indoorEnduses =
          new IndoorEnduses(wrrSim, randomNum / 100.0, sizeOfHousehold, builtYear, scenario);
      ...
      if (isReclaimed) {
        ...
        neighbors =
            city.getVonNeumannNeighbors(
                newXCor, newYCor, neighborDist, Grid2D.BOUNDED, neighbors, xPos, yPos);
        // System.out.println(neighbors.size());
        for (int j = 0; j < neighbors.size(); j++) {
          socialNetwork.addEdge(
              user, neighbors.get(j), new Double(0.25)); // wrrSim.random.nextDouble()
        }
      }
    ...
    outputStreamPopGrowth.println(
        wrrSim.schedule.getTime()
            + " "
            + bag.size()
            + " "
            + (increasedPop * (time % 120) + population[g]));
